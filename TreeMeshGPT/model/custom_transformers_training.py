...
class Attend(Module):
    ...
    def flash_attn(
        ...
        def custom(b, h, q_idx, kv_idx):
            mask_latent = kv_idx < LATENT_LEN
            mask_causal = q_idx >= kv_idx

            return mask_causal | mask_latent
        
        block_mask = create_block_mask(custom, B=None, H=None, Q_LEN=q.shape[-2], KV_LEN=k.shape[-2])
...
class FlexAttentionTransformers(AttentionLayers):
