...
class CloudEncoder(Module):
    def __init__(
        ...
        self.Q=nn.Parameter(torch.randn(M, C))
    ...
    def forward(self, pc: Tensor) -> Tensor:
        B, N, D = pc.shape
        pc_embeddings = self.point_embed(pc) # [b,Nv,C]
        sampled_pc_embeddings = self.Q.unsqueeze(0).expand(B, -1, -1)
